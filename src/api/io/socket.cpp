...
Socket::Socket(int fd, bool connection)
    : _fd(fd)
...
{
    if (_fd != -1) {
        Log::i("closing " + std::to_string(_fd));
        ::close(_fd);
    }
}
...
{
    try {
        auto result = ::sendfile(_fd, file.fd(), nullptr, file.size() - file.offset());

        if (result == -1) {
            if (!is_blocking()) {
                if (errno != EAGAIN)
                    throw std::runtime_error("Error when writing to socket with fd = " + std::to_string(_fd) + " errno = " + std::to_string(errno));
            }
        }
...
T Socket::Read(std::size_t size)
{
    ...
    try {
        ...
        return available;
            ...
            if (available != readBytes)
                throw std::runtime_error("Socket read error on fd = " + std::to_string(_fd) + "."
                                                                                              "Expected to read " + std::to_string(available) + " bytes, but could only read " + std::to_string(readBytes) + " bytes");
            ++_reads;
            return result;
        }
        else {
        ...
        throw std::runtime_error(
            "Caught system error while reading from socket. Text: " + std::string(ex.what()));
    }
}
...
std::string Socket::ReadUntil(const std::string& until, bool peek)
{
    ...
    do {
        char buf;
        auto bytesRead = ::read(_fd, &buf, 1);
        if (bytesRead == -1) {
            if (errno != EAGAIN && errno != EWOULDBLOCK)
                    throw std::runtime_error("Error when reading from socket, errno = " + std::to_string(errno));
            else
            {
                if (-1 != result.find(until.c_str(), 0, until.size()))
                    break;
                else throw std::runtime_error("could not read from socket. connection was probably closed. fd = " + std::to_string(_fd));
            }
            int err = errno;
        }
        if (bytesRead == 0) {
            if (-1 != result.find(until.c_str(), 0, until.size()))
                ...
                throw std::runtime_error("could not read from socket. connection was probably closed. fd = " + std::to_string(_fd));
        }
        result += buf;
        auto found = result.find(until.c_str(), 0, until.size());
        if(-1 != found)
            break;
    } while(true);
    return result;
